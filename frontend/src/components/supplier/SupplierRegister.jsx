...
const SupplierRegister = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post("http://localhost:3333/supplier/auth/register", details)
            .then((res) => {
                console.log(res);
                if (res.status === 200) {
    ...
    return (
        <div className="h-full w-full mx-auto px-6 pb-6 max-w-7xl">
            ...
            <main className="mx-auto max-w-2xl p-6">
                <Link to="/supplier" className="flex items-center gap-4 text-lg text-left">
                ...
                <form className="flex flex-col items-start my-16 mx-auto w-full max-w-xs" onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-col items-start mt-4 w-full">
                        ...
                        <div className="mt-2 w-full">
                            <input className="w-full py-1 px-2 border border-black" type="password" id="password" value={details.password} onChange={handleChange} required />
                        </div>
                    </div>
                    <div className="flex flex-col items-start mt-4 w-full">
                    ...
                    <div className="flex flex-col items-start mt-4 w-full">
                        ...
                        <div className="mt-2 w-full">
                            <input className="w-full py-1 px-2 border border-black" id="phone" value={details.phone} type="number" onChange={handleChange} required />
                        </div>
                    </div>
                    <div className="mt-8 w-full">
                        <button className="w-full py-2 px-4 bg-gray-300 hover:bg-gray-400" type="submit">Register</button>
                    </div>
                    <Link to="/supplier/login" className="mt-8 w-full text-left">Already a user?&nbsp;&nbsp;&nbsp;Login now <i className="fal fa-long-arrow-right inline-block"></i></Link>
                </form>
            </main>
        </div>
    );
