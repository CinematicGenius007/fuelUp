...
const DistributorLogin = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.post("http://localhost:3333/distributor/auth/login", credentials)
            .then((res) => {
                localStorage.setItem("token", res.data.token);
                navigate(`/distributor/${res.data.user._id}`);
            })
            .catch((err) => {
                console.log(err);
            });
        ...
        <div className="h-full w-full mx-auto px-6 pb-6 max-w-7xl">
            ...
            <main className="mx-auto max-w-2xl p-6">
                ...
                <form className="flex flex-col items-start my-16 mx-auto w-full max-w-xs" onSubmit={handleSubmit}>
                    ...
                    <Link to="/distributor/register" className="mt-8 w-full text-left">New here?&nbsp;&nbsp;&nbsp;Register now <i className="fal fa-long-arrow-right inline-block"></i></Link>
                </form>
            </main>
        </div>
    );
